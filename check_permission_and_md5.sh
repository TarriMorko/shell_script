...
DIRECTORY_YOU_WAT_TO_CHECK_PERMISSION="/etc"
...
DIRECTORY_YOU_WAT_TO_CHECK_MD5="/etc"
...
diff_permission_today_with_BASE() {
  diff ${BASE_PERMISSION} $_permission_today -q >/dev/null 2>&1
  if [[ $? -eq 0 ]]; then
    echo ''
...
create_base_md5() {
  ...
  for dir in $DIRECTORY_YOU_WAT_TO_CHECK_MD5; do
    ...
    if [[ $OS = "Linux" ]]; then
      ...
      time find $dir -type f -exec csum -h MD5 {} \; >>$BASE_MD5
    fi
...
create_md5_today() {
  ...
  for dir in $DIRECTORY_YOU_WAT_TO_CHECK_MD5; do
    ...
    if [[ $OS = "Linux" ]]; then
      ...
      time find $dir -type f -exec csum -h MD5 {} \; >>$_md5_today
    fi
  done
...
diff_md5_today_with_BASE() {
  diff ${BASE_MD5} $_md5_today -q >/dev/null 2>&1
